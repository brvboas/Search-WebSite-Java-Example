<%-- 
    Document   : userManagerPage
    Created on : 01/06/2010, 16:02:16
    Author     : Takeshi
--%>
...
<html>
    <head>
        <script src="js/userManagerFunctions.js" type="text/javascript"></script>
    ...
    <body>
        ...
        <form id="formPublication" method="post" action="" name="formPublication" >
            <table width="723" cellspacing="0" border="0" >
                <tr>
                    <td width="265"><img src="img/Usuarios.jpg" width="294" height="418" alt="UsuarioImagem" /></td>
                    <td width="600">
                        <p align="center">Usuário: <input type="text" name="pesquisaADM" size="50px" maxlength="580" id="pesquisaADM">&nbsp;<input type="button" onclick="retornaPesquisa(pesquisaADM.value);" name="btnPesquisa" id="btnPesquisa" value="Buscar"></p>
            ...
            if (request.getParameter("busca").equals("null")) {
                lista = usuario.retornaUsuarios();
            } else {
                pesquisa = request.getParameter("busca");
                lista = usuario.retornaUsuarios(pesquisa);
            }
            int nPaginas;
            if (lista.size() % 10 == 0) {
            ...
            for (int i = start; i < tamPagina; i++) {
                request.setAttribute("ID" + (lista.get(i)).getIdusuario(), lista.get(i));

                            %>

                            <tr>
                                <td width="51" align="center"><%= i + 1%></td>
                                <td width="400"><a href="userDescriptionPage.jsp?id=<%= (lista.get(i)).getIdusuario()%>" title="" onclick=""><%= lista.get(i).getNome()%></a></td>
                                <td width="72"> | <a href="modifyPerfil.jsp?id=<%= (lista.get(i)).getIdusuario()%>" title="" onClick="">Alterar</a> | <a href="#" title="" onClick="javascript:remover2('<%= (lista.get(i)).getIdusuario()%>')">Excluir</a></td>
